...
Keys:
  ...
  p                 — cycle palette
  j                 — toggle Julia mode
  o                 — arm orbit target at center
  Enter             — fire orbit at armed target
  c                 — clear orbit
  [ / ]             — iterations down / up
  1-6               — presets
  0                 — reset view
...
# ── Gold master interaction tuning ───────────────────────────────────────────
ORBIT_STEP_DELAY = 0.090
ORBIT_STEP_POINTS = 1
ORBIT_NUDGE_RATIO = 0.01
DEFAULT_LAUNCH_THEME = os.environ.get("RTA_FRACTAL_THEME", "").strip().lower()
...
if _NUMBA:
    ...
    def _jit_buddhabrot(xmin,xmax,ymin,ymax,W,H,max_iter, zoom=1.0):
        ...
        for s in _prange(samples):
            ...
            h1 = np.uint64(s + ORBIT_STEP_POINTS) * np.uint64(0x9E3779B97F4A7C15)
            h1 ^= (h1 >> np.uint64(30))
...
if _MLX:
    ...
    def compute_julia(C, jc, max_iter, smooth=True, xmin=0,xmax=0,ymin=0,ymax=0,W=0,H=0,zoom=0):
        ...
        if _NUMBA:
            return _jit_julia(xmin, xmax, ymin, ymax, max(W,2), max(H,2), max_iter, jc.real, jc.imag)
...
def dynamic_tui_iters(max_iter, zoom):
    ...
    return min(max_iter, max(floor, int(20 * math.log2(max(zoom, 1) + ORBIT_STEP_POINTS))))
...
def apply_ordered_dither(rgb):
    ...
    h, w, _ = rgb.shape
    tile = np.tile(bayer, (h // 4 + 1, w // 4 + ORBIT_STEP_POINTS))[:h, :w]
...
def accumulate_global_buddhabrot(state, W, H, passes=1, samples_per_pass=None, cache=None):
    ...
    for _ in range(max(1, passes)):
        seed = (cache["passes"] + ORBIT_STEP_POINTS) * 2654435761 + pixel_count + int(zoom)
...
def make_state(mode="mandelbrot"):
    ...
    requested_theme = DEFAULT_LAUNCH_THEME
...
class FractalTUI:
    ...
    def _block_render(self, cols, rows):
        ...
        # Paint orbit target / orbit trace directly into the live pixel buffer.
        def _to_px(zr, zi):
            ix = int((zr - xmin) / max(xmax - xmin, 1e-300) * (W_px - 1))
            iy = int((zi - ymin) / max(ymax - ymin, 1e-300) * (H_px - 1))
            return ix, (H_px - 1) - iy

        def _paint_dot(ix, iy, color):
            if 0 <= ix < W_px and 0 <= iy < H_px:
                rgb[iy, ix] = color

        def _paint_orbit_dot(ix, iy, color):
            for dx, dy in ((0,0), (1,0), (0,1), (1,1)):
                _paint_dot(ix + dx, iy + dy, color)

        def _paint_cross(ix, iy, color):
            for dx, dy in ((0,0), (-1,0), (1,0), (0,-1), (0,1)):
                _paint_dot(ix + dx, iy + dy, color)

        if self.orbit_target is not None and self.orbit_armed:
            tx, ty = _to_px(*self.orbit_target)
            _paint_cross(tx, ty, np.array([255, 220, 90], dtype=np.uint8))

        if self.orbit:
            pts, esc = self.orbit
            orbit_rgb = np.array([90, 255, 150], dtype=np.uint8) if not esc else np.array([255, 110, 110], dtype=np.uint8)
            for zr, zi in pts:
                ix, iy = _to_px(zr, zi)
                _paint_orbit_dot(ix, iy, orbit_rgb)
    ...
    def _header(self, cols):
        ...
        style_name = s.get("visual_style", "classic")
        _, active_style, active_palette = render_style_params(s)
        phosphor_mode = active_palette == "white_phosphor" or style_name == "phosphor"

        if phosphor_mode:
            FG = "\x1b[1m\x1b[38;2;228;255;250m"
            ACC = "\x1b[38;2;132;236;242m"
            DIM = "\x1b[38;2;88;134;142m"
            AMB = "\x1b[38;2;255;196;74m"
            GRN = "\x1b[38;2;175;255;220m"
            RESET = "\x1b[0m"
            ...
            left = (
                f"{FG}{FRACTAL_LABELS[s['mode']]}{tags}"
                f"{DIM}  ·  {ACC}{zstr}"
                f"{DIM}  ·  {FG}{s['max_iter']}it"
                f"{DIM}  ·  {ACC}{PALETTE_LABELS[s['palette']]}{RESET}"
            )
            # No full-width background fill in phosphor mode; this removes the bright horizontal band.
            return left + RESET
    ...
    def _sidebar(self, rows):
        ...
        # ── Colors ────────────────────────────────────────────────────────
        if phosphor_mode:
            BG   = "\x1b[48;2;6;12;16m"            # deep blue-black panel
            BG2  = "\x1b[48;2;8;18;24m"            # slightly brighter rack fill
            BDR  = "\x1b[38;2;132;236;242m"        # cold phosphor border
            DIM  = "\x1b[38;2;112;154;162m"        # muted phosphor labels
            CRT  = "\x1b[1m\x1b[38;2;228;255;250m" # bright phosphor text
            ICE  = "\x1b[38;2;132;236;242m"        # cyan accent
            AMB  = "\x1b[38;2;255;196;74m"         # amber warnings
            GRN  = "\x1b[38;2;175;255;220m"        # healthy / trapped
            RED2 = "\x1b[38;2;255;120;145m"        # escaped / danger
            KEY  = "\x1b[1m\x1b[38;2;220;250;255m"
            TITLE = "PHOSPHOR RACK"
            MODE_TAG = f"{ICE}ACTIVE"
        else:
            BG   = "\x1b[48;2;25;0;0m"
            BG2  = BG
            BDR  = "\x1b[38;2;122;10;10m"
            DIM  = "\x1b[38;2;160;50;50m"
            CRT  = "\x1b[1m\x1b[38;2;255;59;59m"
            ICE  = CRT
            AMB  = "\x1b[38;2;255;191;0m"
            GRN  = "\x1b[38;2;55;255;127m"
            RED2 = "\x1b[38;2;255;80;80m"
            KEY  = "\x1b[1m\x1b[38;2;255;59;59m"
            TITLE = "CRT FRACTAL EXPLORER"
            MODE_TAG = f"{CRT}CRT"

        RST  = "\x1b[0m"

        def line(content_plain, content_ansi, bg_fill=None):
            inner = W - 4
            pad = max(0, inner - len(content_plain))
            fill = BG if bg_fill is None else bg_fill
            return f"{fill}{BDR}║{RST}{fill} {content_ansi}{' '*pad} {BDR}║{RST}"

        def sep():
            return f"{BG}{BDR}╟{'─'*(W-2)}╢{RST}" if phosphor_mode else f"{BG}{BDR}╠{'═'*(W-2)}╣{RST}"

        def blank(bg_fill=None):
            fill = BG if bg_fill is None else bg_fill
            return f"{fill}{BDR}║{' '*(W-2)}║{RST}"
        ...
        def val(txt_plain, txt_ansi=None):
            if txt_ansi is None:
                txt_ansi = f"{CRT}{txt_plain}"
            return line(txt_plain, txt_ansi, BG2 if phosphor_mode else BG)

        def badge(left_plain, left_ansi, right_plain="", right_ansi=""):
            inner = W - 4

            # Keep the right status tag visible and shrink the left title first.
            if right_plain:
                max_left = max(0, inner - len(right_plain) - 1)
                left_plain_fit = left_plain[:max_left]
                left_ansi_fit = left_ansi[:]
                spacer = max(1, inner - len(left_plain_fit) - len(right_plain))
                plain = f"{left_plain_fit}{' ' * spacer}{right_plain}".rstrip()
                ansi = f"{left_ansi_fit}{' ' * spacer}{right_ansi}".rstrip()
            else:
                left_plain_fit = left_plain[:inner]
                plain = left_plain_fit
                ansi = left_ansi

            return line(plain, ansi, BG2 if phosphor_mode else BG)
        ...
        if _MLX and zoom < 800:
            ...
            eng_ansi  = f"{ICE}numba {DIM}float64" if phosphor_mode else f"{CRT}numba {DIM}float64"
        else:
        ...
        active_style_label = active_style["label"]
        active_palette_label = PALETTE_LABELS[active_palette]

        sidebar_lines = [
            top(),
            badge(TITLE, f"{CRT}{TITLE}", "SYNC", f"{MODE_TAG}"),
            sep(),
            blank(),
            label("MODE"),
            val(mode_plain),
            ...
            blank(),
            label("STYLE"),
            val(active_style_label, f"{ICE if phosphor_mode else CRT}{active_style_label}"),
            blank(),
            label("PALETTE"),
            val(active_palette_label, f"{CRT}{active_palette_label}"),
            sep(),
            blank(),
            label("CENTER"),
            val(f"re {cx:+.6f}"),
        ...
        def kline(k, desc):
            plain = f"{k:<6}{desc}"
            accent = ICE if phosphor_mode else KEY
            ansi = f"{accent}{k:<6}{RST}{BG}{DIM}{desc}"
    ...
    def redraw(self):
        ...
        # Clear stale ANSI/background state from prior frames so phosphor mode
        # doesn't leave a horizontal band behind older full-width header fills.
        out = [t.home, t.clear_eos, self._header(cols), t.clear_eol, "\n"]
        body_h = rows - 2
        for i in range(body_h):
            fl = frac_lines[i] if i < len(frac_lines) else ""
            sl = sidebar[i]    if i < len(sidebar)     else ""
            out.append(fl + sl + t.clear_eol + "\n")
    ...
    def handle_mouse(self, key):
        t=self.term
        cols,rows=t.width,t.height
        fractal_cols=max(8, cols-34)
        try:
            me=key._mode_values
            btn=getattr(me, "button_value", None)
            if getattr(me, "released", False):
                return

            # Try several common blessed mouse attribute names.
            mx = None
            my = None
            for name in ("mouse_x", "x", "posx", "rx", "col"):
                if hasattr(me, name):
                    mx = getattr(me, name)
                    break
            for name in ("mouse_y", "y", "posy", "ry", "row"):
                if hasattr(me, name):
                    my = getattr(me, name)
                    break

            # If mouse coordinates are unavailable, keep old center-based wheel zoom.
            if mx is None or my is None:
                cx=(self.state["xmin"]+self.state["xmax"])/2
                cy=(self.state["ymin"]+self.state["ymax"])/2
                if btn==64:
                    zoom_at(self.state,cx,cy,1.3); self._status="scroll zoom in"; self._dirty=True
                elif btn==65:
                    zoom_at(self.state,cx,cy,1/1.3); self._status="scroll zoom out"; self._dirty=True
                return

            # Ignore clicks in the sidebar/footer/header.
            if mx >= fractal_cols or my <= 0 or my >= rows-1:
                return

            px_w=max(self.state["xmax"]-self.state["xmin"], 1e-300)
            px_h=max(self.state["ymax"]-self.state["ymin"], 1e-300)
            cx=self.state["xmin"] + (mx / max(fractal_cols-1,1)) * px_w
            cy=self.state["ymax"] - ((my-1) / max(rows-3,1)) * px_h

            if btn==64:
                zoom_at(self.state,cx,cy,1.3); self._status=f"zoom in @ {cx:+.5f}{cy:+.5f}i"; self._dirty=True
            elif btn==65:
                zoom_at(self.state,cx,cy,1/1.3); self._status=f"zoom out @ {cx:+.5f}{cy:+.5f}i"; self._dirty=True
            else:
                self._status=f"mouse preview @ {cx:+.6f}{cy:+.6f}i  use o to arm target"
                self._dirty=True
        except Exception as e:
            self._status=f"mouse error: {e}"
            self._dirty=True
            ...
            with t.cbreak(), t.hidden_cursor(), t.mouse_enabled(report_drag=False):
                while True:
                    try:
                        ...
                        self._dirty=False
                        sys.stdout.write(t.home + t.clear_eos)
                    ...
                    if k in ('q','Q'): break
                        ...
                        if self.orbit_armed and self.orbit_target is not None:
                            span = self.state["ymax"] - self.state["ymin"]
                            self.orbit_target = (self.orbit_target[0], self.orbit_target[1] + span * ORBIT_NUDGE_RATIO)
                            cx,cy=self.orbit_target
                            self._status=f"target @ {cx:+.6f}{cy:+.6f}i  enter fire"
                            self._dirty=True
                        else:
                        ...
                        if self.orbit_armed and self.orbit_target is not None:
                            span = self.state["xmax"] - self.state["xmin"]
                            self.orbit_target = (self.orbit_target[0] - span * ORBIT_NUDGE_RATIO, self.orbit_target[1])
                            cx,cy=self.orbit_target
                            self._status=f"target @ {cx:+.6f}{cy:+.6f}i  enter fire"
                            self._dirty=True
                        else:
                        ...
                        reset_view(self.state); self.orbit=None; self.orbit_target=None; self.orbit_armed=False; self.orbit_anim_pts=None; self.orbit_anim_idx=0; self._bb_cache.clear(); self._dirty=True
                        ...
                        new_style = style_keys[(idx + ORBIT_STEP_POINTS) % len(style_keys)]
                        ...
                        if self.state["julia_mode"]:
                            if self.state["mode"]=="buddhabrot":
                                self.state["mode"]="mandelbrot"
                            self.state.update(xmin=-1.5,xmax=1.5,ymin=-1.5,ymax=1.5,zoom=1.0)
                            self._status=(f"JULIA ON  c={self.state['julia_c'].real:+.4f}"
                                          f"{self.state['julia_c'].imag:+.4f}i  "
                                          "o arm target  /  enter fire")
                        else:
                            reset_view(self.state)
                            self._status="JULIA OFF  o arm target  /  enter fire"
                        self.orbit=None
                        self.orbit_target=None
                        self.orbit_armed=False
                        self.orbit_anim_pts=None
                        self.orbit_anim_idx=0
                        ...
                        if self.orbit_target is None:
                            cx=(self.state["xmin"]+self.state["xmax"])/2
                            cy=(self.state["ymin"]+self.state["ymax"])/2
                            self.orbit_target=(cx,cy)
                        self.orbit_armed=True
                        self.orbit=None
                        self.orbit_anim_pts=None
                        self.orbit_anim_idx=0
                        cx,cy=self.orbit_target
                        self._status=(f"target armed @ {cx:+.6f}{cy:+.6f}i  "
                                      f"arrows move  enter fire  c clear")
                        self._dirty=True
                    elif k in ('\n','\r') and self.orbit_armed:
                        cx,cy=self.orbit_target
                        jc=self.state["julia_c"] if self.state["julia_mode"] else None
                        pts,esc=orbit_pts(cx,cy,self.state["max_iter"],self.state["mode"],jc)
                        self.orbit_anim_pts=pts
                        self.orbit_anim_esc=esc
                        self.orbit_anim_idx=1
                        self.orbit_anim_last=time.time()
                        self.orbit=(pts[:1],esc)
                        self.orbit_armed=False
                        self._status=(f"tracing orbit @ {cx:+.6f}{cy:+.6f}i  "
